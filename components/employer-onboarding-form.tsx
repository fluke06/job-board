...
export function EmployerOnboardingForm() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (res.status === 409) {
      form.setError("slug", { message: "That URL is taken — try another" });
      return;
    }
    const err = await res.json().catch(() => ({}));
    toast.error(err?.error ?? "Could not create company");
  }
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Your company page lives at <code>/companies/{"{slug}"}</code>.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  rows={4}
                  placeholder="A one or two sentence pitch for candidates."
                  {...field}
                />
              </FormControl>
